...
dojo.require("myModules.idescat");
  ...
  if (configOptions.appid || (urlObject.query && urlObject.query.appid)) {
    ...
    var requestHandle = esri.request({
      ...
      load: function (response) {
        ...
        if( response.values.idescat !== undefined)
        {
          configOptions.idescat = response.values.idescat;
        }
...
function createApp() {
  ...
  if (urlObject.query) {
    ...
    if( urlObject.query.idescat)
    {
      configOptions.idescat = (urlObject.query.idescat === 'true') ? true : false;
    }
  ...
  if( configOptions.idescat === 'true' || configOptions.idescat === true) {
    configOptions.idescat = true;
  }  else  {
    configOptions.idescat = false;
  }
...
function initUI(response) {
  ...
  if( configOptions.idescat === 'true' || configOptions.idescat === true )
  {
    var idescatTb = new dijit.form.ToggleButton({
      showLabel: true,
      label: "idescat",
      title: "idescat",
      checked: false,
      iconClass: 'idescatIcon',
      id: 'idescatButton'
    }, dojo.create('div'));
    dojo.byId('webmap-toolbar-center').appendChild(idescatTb.domNode);
    
    new myModules.idescat( {map: map, button: idescatTb});
  }
